...
def detect(save_img=False):
    ...
    pd = pds.read_csv("/opt/camera-sc2/master_key-qi-new.csv") #master key berisikan kunci untuk setiap model bemper (jenis, warna, dll)
    analytic_path = f"/opt/camera-sc2/analytic/analytic_qi"
    clockdb = f"/opt/camera-sc2/clockDB-qi.txt"
    checkStuckLog = f"/opt/camera-sc2/checkStuckQI.txt"
    checkStuckLogbckup = f"/opt/camera-sc2/checkStuckQILast.txt"
    fps, w, h = 15, 640, 360
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                z.write(str(check_stuck))
                z.close()
                print("\n[DEBUG10]", video_date, video_time, check_stuck)
                if time.time()-timer>4: #Tunggu sekitar 2 detik untuk memastikan frame benar benar kosong
                    if centers2:  
                        ...
                        if num_data>0:                  
                            ...
                            print("\n[INFO] ",detected_objects, " ??? [PATTERN] ",pattern_result)
            ...
            if view_img or True:
                ...
                if time.time() - previous_image_time>20:#digunakan untuk mengantisipasi terdapat freeze kamera, maka program akan dipaksa break, dan di restart program
                    if np.array_equal(im0, previous_image):
                        ...
                        cv2.destroyAllWindows()
                        with open('conection-log.txt','a') as file:
...
#     #os.system("echo {password} | sudo -S /opt/camera-sc2/start-broadcast-Qi.sh")
#     while True:
#         if shape_vir:
#             w_vir=shape_vir[1]
#             h_vir=shape_vir[0]
#             with pyvirtualcam.Camera(width=w_vir, height=h_vir, fps=60, device='/dev/video4') as cam:
#                 while True:
...
def updateConnectivity():
    while True:
        os.system("echo $(date +%s) > /opt/camera-sc2/updateConnQI")
        time.sleep(5)                
