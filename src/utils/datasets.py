...
def change_hsv(img, hue = None, sat = None, val = None):
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):
            ...
            # #cap = cv2.VideoCapture("/dev/stechoqCamera")
            # cap = cv2.VideoCapture("/dev/v4l/by-id/usb-Huawei_HiCamera_12345678-video-index0")
    ...
    def update(self, index, cap):
        ...
        while cap.isOpened():
            ...
            gr = cap.grab()
            if not gr:
                print("FAIL GRAB CAMERA")
                now = datetime.now()
                _base = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # root deployment (parent src/)
                with open(os.path.join(_base, "data", "logs", "conection-log.txt"),'a') as file:
                            con_log= now.strftime("%Y-%m-%d %H:%M:%S") + "  CAMERA got FRAME FREEZE, check LAN connection!!"
                            file.write(con_log + '\n')
                raise StopIteration
            if n == 4:  # read every 4th frame
                success, im = cap.retrieve()
                if not success:
                    print("FAIL RETRIEVE CAMERA")
                    raise StopIteration
                # im = cv2.resize(im, (640,360), interpolation = cv2.INTER_CUBIC)
                im = change_hsv(im, val=-10)
